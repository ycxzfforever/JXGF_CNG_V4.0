...
void LockDataStore(void)
{
    ...
    At24C512WriteNByte(LOCK_ADDR,&Password[0],6);		//写入主板外扩EE
    MsDelay(100);
    At24C128WriteNByte(0,        &Password[0],6);		//写入加密板EE
    
    if(At24C128ReadNByte(0, buf, 6))					//读取加密板数据对比
    {
        for(i1=0;i1<6;i1++)
        {
    ...
    if(At24C512ReadNByte(LOCK_ADDR, tmp, 6))			//读取主板外扩EE加密数据对比
    {
        for(i2=0;i2<6;i2++)
        {
    ...
    if((i1==6)&&(i2==6))								//判断两者读取数据是否完全正确
    {
        if(SaveSysparas(0,(uint8_t*)&sysparas.head,sizeof(Sysparas)))
        {
            SetSuccess();
        }
		else
